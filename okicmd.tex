...
% hyperref
\usepackage[bookmarksnumbered,pdfdisplaydoctitle,pdfusetitle]{hyperref}
...
\subsection{Options}
Options can be passed as \cs{usepackage}\Oarg{\option{option1},\option{option2}}\Marg{okicmd}.
...
Possible size options are \cs{normal}, \cs{big}, \cs{Big}, \cs{bigg}, and \cs{Bigg}.
If no size option is passed, parentheses' size is adjusted automatically by \cs{left} and \cs{right}.
...
\begin{center}
  \begin{tabular}{lcl} \toprule
    ...
    \cs{floor}\Marg{\cs{cdot}}                 & $\floor{\cdot}$        & \cs{left}\cs{lfloor}\cs{cdot}\cs{might}\cs{rfloor}                             \\
    \cs{ceil}\Marg{\cs{cdot}}                  & $\ceil{\cdot}$         & \cs{left}\cs{lceil}\cs{cdot}\cs{right}\cs{rceil}                              \\
    \cs{pprn}\Marg{\cs{cdot}}                  & $\pprn{\cdot}$         & \code{\cs{left}(\cs{left}(\cs{cdot}\cs{right})\cs{right})}                         \\
    \cs{ccurl}\Marg{\cs{cdot}}                 & $\ccurl{\cdot}$        & \code{\cs{left}\cs{}\{\cs{left}\cs{}\{\cs{cdot}\cs{right}\cs{}\}\cs{right}\cs{}\}} \\
    \cs{ssqbr}\Marg{\cs{cdot}}                 & $\ssqbr{\cdot}$        & \code{\cs{left}\lbrack\cs{left}\lbrack\cs{cdot}\cs{right}\rbrack\cs{right}\rbrack} \\
    \cs{aagbr}\Marg{\cs{cdot}}                 & $\aagbr{\cdot}$        & \begin{tabular}[l]{@{}l@{}}\cs{left}\cs{langle}\cs{left}\cs{langle}\\\code{ \cs{cdot}\cs{right}\cs{rangle}\cs{right}\cs{rangle}}\end{tabular} \\
    \bottomrule
  \end{tabular}
\end{center}
...
\begin{center}
  \begin{tabular}{lcl} \toprule
    \multicolumn{1}{c}{Input}                               & Output                   & \multicolumn{1}{c}{\LaTeX\ (almost) equivalent}     \\\midrule
    \cs{set}\Marg{a, b, c}                                  & $\set{a, b, c}$          & \code{\cs{left}\cs{\{}a, b, c\cs{\}}} \\
    ...
    \cs{N}                                                  & $\N$                     & \cs{mathbb}\Marg{N}                                 \\
    \cs{Z}                                                  & $\Z$                     & \cs{mathbb}\Marg{Z}                                 \\
    \cs{Q}                                                  & $\Q$                     & \cs{mathbb}\Marg{Q}                                 \\
    \cs{R}                                                  & $\R$                     & \cs{mathbb}\Marg{R}                                 \\
...
\begin{center}
  \begin{tabular}{lcl} \toprule
    ...
    \code{\cs{Det} A}                         & $\Det A$                     & \code{\cs{operatorname}\Marg{Det} A}            \\
    \code{\cs{Pf} A}                          & $\Pf A$                      & \code{\cs{operatorname}\Marg{Pf} A}             \\
    \code{\cs{perm} A}                        & $\perm A$                    & \code{\cs{operatorname}\Marg{perm} A}           \\
    \code{\cs{Hf} A}                          & $\Hf A$                      & \code{\cs{operatorname}\Marg{Hf} A}             \\
    ...
    \cs{ones}                                 & $\ones$                      & \code{\cs{mathds}\Marg{1}}                      \\
    \cs{norm}\Marg{x}                         & $\norm{x}$                   & \code{\cs{left}\cs{}|x\cs{right}\cs{}|} \\
...
Spaces after ``arg'' in \cs{argmin} and \cs{argmax} are omitted by passing the \option{noargminspace} option.
...
\begin{center}
  \begin{tabular}{lcl} \toprule
    ...
    \cs{order}\Darg{n}                                        & $\order(n)$              & \code{\cs{mathrm}\Marg{o}(n)}          \\
    \cs{poly}\Darg{n}                                         & $\poly(n)$               & \code{\cs{operatorname}\Marg{poly}(n)}       \\
    \cs{polylog}\Darg{n}                                      & $\polylog(n)$            & \code{\cs{operatorname}\Marg{polylog}(n)}    \\
    \bottomrule
  \end{tabular}
\end{center}
